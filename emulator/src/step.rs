        ...
        match self.next_byte()? {
            ...
            0x13 => {
                ...
                let (dst, src) = Register::pair_from(&self.next_byte()?)
                    .map_err(EmulationError::RegisterIndexError)?;
                self.memory
                    .write_byte(self.cpu.register(dst), self.cpu.register(src) as u8)?;
            }
            0x14 => {
                // STW addr
                let addr = self.next_word()?;
            ...
            0x17 => {
                ...
                let (dst, src) = Register::pair_from(&self.next_byte()?)
                    .map_err(EmulationError::RegisterIndexError)?;
                self.memory
                    .write_word(self.cpu.register(dst), self.cpu.register(src))?;
            }
            op @ 0x18..=0x1B => {
                // ST addr, reg
                let addr: u16 = self
            ...
            op @ (0x28 | 0x30 | 0x38) => {
                ...
                match op & !0x07 {
                    0x28 => (),
                    0x30 if !ConditionCode::try_from(self.next_byte()?)
                        .map_err(EmulationError::InvalidCondition)?
                        .meets(self.cpu.flags) =>
                    {
                        return Ok(());
                    }
                    0x38 => {
                ...
                self.cpu.pc = self
                    .cpu
                    .pc
                    .wrapping_add_signed(self.next_byte()? as i8 as i16);
            }
            op @ (0x29 | 0x31 | 0x39) => {
                // JMP addr
                match op & !0x07 {
